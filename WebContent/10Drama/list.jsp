    ...
    <body>
        ...
        <table class="tbl" id="drama">
            ...
            try{
             	...
             	// 다이나믹 웹 프로젝트에서는 WEB-INF > ilb 폴더에 넣는다
             	...
             	// - 쿼리문을 DB에 보낼 상태 완료
             	// - 중간에 쿼리문에 넣을 값을 추가할 수 있다
             	
             	// 12. 쿼리를 DB에 전송하여 실행 후 결과집합(결과셋)을 가져온다
				// ResultSet객체는 DB에서 쿼리결과를 저장하는 객체

				rs = pstmt.executeQuery();
             	// executeQuery() 쿼리 실행 메서드

             	// 13. 저장된 결과집합의 레코드 수 만큼 돌면서 코드 만들기
             	// 돌아주는 제어문 : while(조건){실행문}
             	// 레코드 유무 체크 메서드 : next()
             	// rs는 ResultSet 객체
             	// rs.next() -> 첫라인 다음 라인이 있으면 true / 아니면 false
             	// 첫 번쨰 라인은 항상 컬럼명이 첫 번째 라인이다
             	// 따라서 다음라인이 있다는 것은 결과 레코드가 있다는 말
             	
             	// 일련번호용 변수
             	int listNum = 1;
             	
             	// 결과셋에 레코드가 있는 동안 계속 순회한다
             	//rs.getString(컬럼명)
             	// -> 문자형일 경우, getString(), 숫자형은getInt()
             	// -> 컬럼명은 DB 테이블에 실제로 생성된 컬럼명이다
             	while(rs.next()){
             		result += "<tr>"
             				+"<td>"+listNum+"</td>"
             				// +"<td>"+rs.getInt("idx")+"</td>"
             				// 일년 번호는 DB의 idx 기본키를 쓰지 않고
             				// 반복되는 동안 순번을 만들어 쓴다
             				+"<td><a href='#'>"+rs.getString("dname")+"</a></td>"
