...
//POST 방식의 한글처리 : 이것 안쓰면 한글깨짐!!!
	...
	try{
		// 수정할 레코드의 유일키인 idx값을 get방식으로 전달 받는다
		// jsp에서는 get방식이든 post방식이든 하나의 메서드로 관리한다
	...
	// executeUpdate() 쿼리실행 메서드 -> delete 실행함     	
	// -> insert나 update 모두 DB가 변경되는 것이므로 executeUpdate() 메서드가 모두 처리한다
