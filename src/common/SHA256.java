...
public class SHA256 {
	...
	// 좀더 보안성이 높다!!!
	private final static String salt = "mysalt";// 영문으로꼭 솔트를!!!
	// 주의사항: 솔트 문자열을 한글과 같은 2byte 문자를 사용하게 되면
	// 웹서버의 문자셋이 조금씩 다를 수 있기때문에 이 솔트문자가 다르게
	// 변형되어서 비번을 생성하게된다!
	// 문제가 되는 것은 서버를 이전해서 이것이 달라질때 기존 비번 사용자들이
	// 다른 패턴으로 생성된 비번으로 인해 로그인이 안되는 사태가 발생할 수 있다!
	// 따라서 솔트 문자열은 꼭 영문으로 해주자!
